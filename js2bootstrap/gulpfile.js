...
function changed(stream, sourceFile, destPath) {
...
gulp.task('images', function() {
	return gulp.src(config.src_images)
		.pipe(plugins.changed(config.dist+'/img', {hasChanged: changed}))
...
gulp.task('sass:dist', function() {
	return gulp.src(config.src_mainScss)
		...
		.pipe(plugins.changed(config.dev+'/css', {hasChanged: changed}))
		.pipe(gulp.dest(config.dev+'/css'))
		.pipe(plugins.size({title: 'sass'}));
});
...
gulp.task('copy', function() {
	...
	// .pipe(plugins.debug({'title': 'copy all'}))
		...
		//.pipe(plugins.debug({'title': 'copy chg'}))
